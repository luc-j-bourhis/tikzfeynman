...
\draw[fermion] (v3) to[semiloop] node[midway, below=0.1cm]{$q$} (v2);
\end{tikzpicture}
\caption{The same diagram again but tweaking some options: the ``wavelength'' of one $Z$ propagator, and the thickness of all lines. Thickness named options are: `ultra thin', `very thin', `thin', `semithick', `thick', `very thick', `ultra thick'. One may also specify a size with `line width=4.5pt' or `1mm' e.g.}
\end{figure}
